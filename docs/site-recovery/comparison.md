...
| Motion system | A blanket reduced-motion stylesheet collapses all animation and the hero sequence completes behind the OAB curtain | Earlier sections used Framer Motion reveal patterns | Remove blanket animation suppression, retain preference-aware reduction, synchronise the hero with curtain fade, and support `?motion=full` for explicit visual QA | `app/globals.css`, `app/components/motion-provider.tsx`, `app/components/page-load-curtain.tsx`, `app/components/hero-section.tsx`, `components/ui/tunnel-background.tsx` |
...
| Homepage order | Story precedes work; a rate catalogue interrupts the narrative | One-page editorial portfolio | Introduction immediately after the hero; selected work follows; then experience, capabilities, work together, newsletter and contact | `app/page.tsx` |
